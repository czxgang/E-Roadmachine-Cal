...
FIXED_PARTS = pd.DataFrame({
    ...
    "Power(kW)": [50, 4, 7.4, 7.5, 8.68, 0, 9.4, 2]
})
...
with st.sidebar:
    ...
    # 计算并显示调整后的总功率
    total_power_all_raw = (parts_df["Power(kW)"] * parts_df["Load Factor"]).sum()
    total_power_all = total_power_all_raw / 0.95  # 液压效率
    st.metric("All parts average power", f"{total_power_all_raw:.2f} kW")
...
# 重新计算调整后的总功率（与侧边栏一致）
total_power_all_raw = (df["Power(kW)"] * df["Load Factor"]).sum()
total_power_all = total_power_all_raw / 0.95

if has_heater:
    normal_df = df[~heater_mask]
    total_power_normal_raw = (normal_df["Power(kW)"] * normal_df["Load Factor"]).sum()
    total_power_normal = total_power_normal_raw / 0.95
else:
...
soc_ocv_df = pd.DataFrame({
    'SOC_%': soc_points,
    'Discharge_OCV': discharge_ocv,
    'Charge_OCV': charge_ocv
})
...
def get_ocv(soc_percent, mode='discharge'):
    """
    根据 SOC (%) 和模式返回开路电压 (V)
    mode: 'discharge' 或 'charge'
    """
    if mode == 'discharge':
        return float(f_discharge(soc_percent))
    else:
        return float(f_charge(soc_percent))
...
with tab2:
    ...
    use_heater_tab2 = st.checkbox("Run Screed Heating(time=0.5h)", value=True, key="heater_tab2")
