...
with tab3:
    ...
    if "charge_power" not in st.session_state:
        st.session_state.charge_power = 160.0  # 默认充电功率
    if "target_soc" not in st.session_state:
        st.session_state.target_soc = 90.0  # 默认目标SOC 90%
    if "slow_ratio" not in st.session_state:
        st.session_state.slow_ratio = 0.5  # 默认降速后充电功率比例
            ...
            if paving_speed <= 0:
                ...
                if use_heater_tab3 and total_usable_energy < energy_heat:
                    ...
                    if work_power <= 0:
                        ...
                        # 所需摊铺时间（不含充电）
                        paving_time = road_length / (paving_speed * 60)  # 小时
                        ...
                        # 降速阈值SOC对应的能量（固定90%）
                        slow_threshold_soc = 90.0
                        slow_threshold_energy = total_usable_energy * (slow_threshold_soc / 100.0)
                        ...
                        work_segments = []  # 记录每次连续工作时间
                        
                        # 辅助函数：计算从当前能量充电到目标能量所需的时间（考虑降速）
                        def compute_charge_time(current_energy, target_energy, total_energy, charge_power, slow_ratio):
                            ...
                            # 分段计算
                            time = 0.0
                            # 阶段1：从当前SOC到 min(target_soc, slow_threshold)
                            if current_soc < slow_threshold and target_soc > current_soc:
                                ...
                                current_soc = end_soc
                            # 阶段2：如果 target_soc > slow_threshold，进入降速段
                        ...
                        # 循环，直到完成摊铺时间
                        while t_worked < paving_time - 1e-6:
                            # 本次可工作的时间（从当前能量到阈值）
                        ...
                        single_charge_time = compute_charge_time(threshold_energy, target_energy, total_usable_energy,
                                                                 st.session_state.charge_power, st.session_state.slow_ratio)
                        if single_charge_time <= 0:
                            # 如果阈值能量大于目标能量（不会发生），则设为0
                        ...
                        st.session_state.sim_result = {
                            ...
                            "energy_per_charge": target_energy - threshold_energy,  # 每次充电需要补充的能量
